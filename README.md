...
6. **iOS crash on reopen (EXC_BAD_ACCESS in plugin registration)** — The native iOS code of `flutter_foreground_task`, `map_launcher`, `path_provider_foundation`, and `shared_preferences_foundation` can crash on cold start (null dereference in `register(with:)`). A Run Script build phase "Patch GeneratedPluginRegistrant (disable crashy plugins on iOS)" in the Runner target comments out their registration lines in `GeneratedPluginRegistrant.m` before compile. If that file is regenerated (e.g. after `flutter pub get`), the script re-applies the patch on the next build. Foreground task is Android-only; map_launcher is unused in lib. For `path_provider`, the only usage (register flow temp dir) falls back to `Directory.systemTemp` on iOS. For `shared_preferences`, the app uses `getAppPrefs()` (see `lib/app/data/helper/AppPrefs.dart`). In **Debug** builds on iOS the script disables the `SharedPreferencesPlugin`, so `getAppPrefs()` falls back to in-memory storage (prefs are not persisted across restarts while debugging). In **Release** builds on iOS the plugin remains enabled, so user session and other preferences are persisted normally.
